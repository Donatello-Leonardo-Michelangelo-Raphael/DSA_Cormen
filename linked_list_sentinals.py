...
def pointer_next(pntr,initial_pointer):
	if(pntr>=len(initial_pointer) or pntr<0):
		return 'error'
	elif(initial_pointer[pntr]=='NULL'):
		return initial_pointer[0] 
	else:
...
def pointer_prev(pntr,initial_pointer):
	if(pntr>len(initial_pointer)-1 or pntr<0):
		return 'error'
	elif(initial_pointer[pntr]==initial_pointer[0]):
		return initial_pointer[len(initial_pointer)-1]
	else:
		return initial_pointer[pntr-1]
